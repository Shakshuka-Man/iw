...
| `starting_stage` | `PlotStage \| None` | `None` | Where the plot begins. Left out, it is deduced: the one stage nothing transitions into. Name it when the transitions cannot say — a plot that loops back to its opening has no such stage, and one you want to begin partway through has several. The stage tracker's initial value is this stage. |
...
### `add_single_plot(world: World, plot_stages: list[PlotStage], plot_transitions: list[PlotTransition], starting_stage: PlotStage | None = None) -> None`

Single-plotline convenience: build one plotline named "Plot" and add it to `world`, in place. For a custom name or trackers, call add_plots with an explicit Plotline (see add_plots). `starting_stage` says where the plot begins; left out, it is deduced from the transitions (see Plotline).
